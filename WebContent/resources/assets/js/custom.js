		...
		.ready(
				function($) {
							...
							.click(
									function() {
										if ($(this).hasClass('seleted')) {
											...
											i++;
											this_item = 'seleted_' + i;
											var name = $(this).next()
													.children().text();
											var desc = $(this).children()
													.children().children()
													.text();
											var itemCode = $(this).next()
													.next().text();
											// console.log(itemCode);
											$('.butnote').css('margin-top',
													'+=45');
											...
											$(this).parent().prev().addClass(
													'ovrlay_' + i);
											$(this).addClass('seleted_' + i);
											$(this).children().children()
													.children().css('opacity',
															'0.95');
											// $(this).parent().prev().children().html('Selected');
											$(this)
													.parent()
													.prev()
													.children()
											...
											$(this)
													.next()
													.append(
															'<span class="top-close remove-item up_remove-item_'
																	+ i
																	+ ' remove-item_'
																	+ i
																	+ '" onclick="remove_item(\'remove-item_'
																	+ i
																	+ '\',\''
																	+ this_item
																	+ '\',\''
											...
											var html = '<div class="list-item">'
													...
													+ '</figure>'
													+ '</div>'
													+ '<span class="right remove-item down_remove-item_'
													+ i
													+ ' remove-item_'
													+ i
													+ '" onclick="remove_item(\'remove-item_'
					...
					if ($('body').hasClass('navigation-fixed')) {
						$('.off-canvas-navigation').css('top',
								-$('.header').height());
						$('#page-canvas').css('margin-top',
								$('.header').height());
					}
					...
					$('.off-canvas-navigation header').css('line-height',
							$('.header').height() + 'px');
					...
					$('.expand-content').live(
							...
							function(e) {
								...
								var parentHeight = $(this).closest(
										'.expandable-content').height();
								var contentSize = $(children + ' .content')
										.height();
					...
					$("#redefine-search-form").load(
							"resources/assets/external/_search-bar.html",
							function() {
								setInputsWidth();
								// autoComplete();
							});
					...
					$('.navigation .nav a[href^="#"], a[href^="#"].roll')
							.on(
									'click',
									function(e) {
										e.preventDefault();
								...
								.each(
										function() {
											...
											if ($(this).attr('data-step')) {
												step = parseInt($(this).attr(
														'data-step'));
											} else {
												step = 10;
											}
											var sliderElement = $(this).attr(
													'id');
											var element = $('#' + sliderElement);
											var valueMin = parseInt($(this)
													.attr('data-value-min'));
											var valueMax = parseInt($(this)
													.attr('data-value-max'));
											$(this).noUiSlider({
...
/*
 * $(window).resize(function(){ adaptBackgroundHeight(); var $equalHeight =
 * $('.equal-height'); for( var i=0; i<$equalHeight.length; i++ ){ equalHeight(
 * $equalHeight ); } });
 */
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Functions
...
function filter(parm) {
	...
	if (parm == "All") {
		$('div.item').parent().hide();
		$('div.item').parent().show();
		$('div.buttons.pull-left > a.btn.icon').removeClass('active');
		$('#All').addClass('active');
	} else {
		$('div.item').parent().hide();
		$('div#' + parm).parent().show();
...
function setInputsWidth() {
	...
	for (var a = 0; a < searchBar.length; a++) {
		...
		if ($('.search-bar.horizontal .form-group label').length > 0) {
			$('.search-bar.horizontal .form-group:last-child button').css(
					'margin-top', 25)
		}
	}
}
...
function rating(element) {
	var ratingElement = '<span class="stars">'
			+ '<i class="fa fa-star s1" data-score="1"></i>'
			+ '<i class="fa fa-star s2" data-score="2"></i>'
			+ '<i class="fa fa-star s3" data-score="3"></i>'
			+ '<i class="fa fa-star s4" data-score="4"></i>'
			+ '<i class="fa fa-star s5" data-score="5"></i>' + '</span>';
	if (!element) {
		element = '';
	}
	$.each($(element + ' .rating'),
			function(i) {
				...
				if ($(this).hasClass('active')) {
					$(this).append(
							'<input readonly hidden="" name="score_'
									+ $(this).attr('data-name')
									+ '" id="score_'
									+ $(this).attr('data-name') + '">');
				}
				...
				for (var e = 0; e < rating; e++) {
					var rate = e + 1;
					$(this).children('.stars').children('.s' + rate).addClass(
							'active');
				}
			});
...
function drawOwlCarousel(_rtl) {
	$.getScript("/cater4party/resources/assets/js/owl.carousel.min.js",
			...
			function(data, textStatus, jqxhr) {
				if ($('.owl-carousel').length > 0) {
					...
					$('.item-slider').on(
							...
							function(event) {
								var thumbnailNumber = $(
										'.item-slider .owl-item.active img')
										.attr('data-hash');
								$(
										'.item-gallery .thumbnails #thumbnail-'
												+ thumbnailNumber).trigger(
										'click');
							});
					return false;
				}
		...
		if (category == 'real_estate') {
			if (json.data[a].item_specific) {
				if (json.data[a].item_specific.bedrooms) {
					itemSpecific += '<span title="Bedrooms"><img src="resources/assets/img/bedrooms.png">'
							+ json.data[a].item_specific.bedrooms + '</span>';
				}
				if (json.data[a].item_specific.bathrooms) {
					itemSpecific += '<span title="Bathrooms"><img src="resources/assets/img/bathrooms.png">'
							+ json.data[a].item_specific.bathrooms + '</span>';
				}
				if (json.data[a].item_specific.area) {
					itemSpecific += '<span title="Area"><img src="resources/assets/img/area.png">'
							+ json.data[a].item_specific.area
							+ '<sup>2</sup></span>';
				}
				if (json.data[a].item_specific.garages) {
					itemSpecific += '<span title="Garages"><img src="resources/assets/img/garages.png">'
							+ json.data[a].item_specific.garages + '</span>';
				}
				return itemSpecific;
			}
			...
			if (json.data[a].item_specific) {
				if (json.data[a].item_specific.menu) {
					itemSpecific += '<span>Menu from: '
							+ json.data[a].item_specific.menu + '</span>';
				}
				return itemSpecific;
			}
			return itemSpecific;
...
function adaptBackgroundHeight() {
	$('.background').each(function() {
		if ($(this).children('img').height() < $(this).height()) {
			// $(this).children('img').css('right', (
			// $(this).children('img').width()/2 - $(window).width())/2 );
