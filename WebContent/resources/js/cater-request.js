...
function populateStateDropDown() {
	...
	var selectNode = $("<select/>").attr('name', 'state').attr('id', 'state').attr('required', 'true').attr('class', 'form-control inputs').appendTo(stateDiv);
	$("<option />", {value: '', text: ''}).appendTo(selectNode);
	for(var i = 0;i<obj.length;i++) {
		$("<option />", {value: obj[i].abbreviation, text: obj[i].name}).appendTo(selectNode);
	}
}
