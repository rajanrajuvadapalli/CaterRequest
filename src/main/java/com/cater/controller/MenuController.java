...
public class MenuController {
	...
	public String viewCompleteMenu(HttpSession httpSession, ModelMap modelMap,
		...
		try {
			...
			List <String> keys = Lists.newArrayList(StringUtils.split(restaurantName, " "));
			keys.add(restaurantZipCode);
			String fileName = StringUtils.join(keys, "_");
			String fileNameExtension = ".json";

			/*
			try {
				File f = new File(MenuController.class.getResource("/menus/full/" + fileName + fileNameExtension).getFile());
				Menu baseMenu = new MenuDeserializer().readJSON(f);
